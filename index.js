...
Telegram: {
Premium: [
{ id: "tp1", name: "Premium Members", price: 50, min: 100, desc: "High quality users.\nDelivery: 20 min – 15 hrs." },
{ id: "tp2", name: "Channel Boost", price: 65, min: 200, desc: "Boost your channel rank." }
],
Followers: [
...
parse_mode: 'Markdown',
reply_markup: [[{ text: "✍️ Reply", callback_data: `arep_${id}` }]]
});
delete sessions[id];
...
// Admin reply
if (s.step === 'REPLYING' && id === ADMIN_ID && s.target) {
...
return bot.sendMessage(id, "💳 Select Amount:", { reply_markup: { inline_keyboard: buttons } });
}

if (data.startsWith('amt_')) {
const amt = parseFloat(data.split('_')[1]);
sessions[id].depositAmount = amt;
const btns = Object.keys(CRYPTO).map(c => [{ text: c, callback_data: `pay_${c}` }]);
btns.push([{ text: "🏠 Main Menu", callback_data: "back_to_menu" }]);
return bot.sendMessage(id, `💳 Amount: $${amt}\nSelect Crypto:`, { reply_markup: { inline_keyboard: btns } });
}

if (data.startsWith('pay_')) {
const coin = data.split('_')[1];
const depId = "D" + Date.now();
db.deposits[depId] = { userId: id, amount: sessions[id].depositAmount, crypto: coin, status: 'pending' };
saveDB();

bot.sendMessage(id, `⚠️ *PAYMENT REQUIRED*\nAmount: $${sessions[id].depositAmount}\nCoin: ${coin}\nAddress: \`${CRYPTO[coin]}\`\nAfter payment click below.`, {
parse_mode: 'Markdown',
reply_markup: { inline_keyboard: [[{ text: "✅ I Have Paid", callback_data: `paid_${depId}` }], [{ text: "🏠 Main Menu", callback_data: "back_to_menu" }]] }
});

// Notify admin
bot.sendMessage(ADMIN_ID, `🚨 *Deposit Submitted*\nUser: ${id}\nAmount: $${sessions[id].depositAmount}\nCoin: ${coin}`, {
parse_mode: 'Markdown',
reply_markup: [
...
dep.status = 'approved';
saveDB();
bot.sendMessage(dep.userId, `✅ Deposit Approved! Balance: $${db.users[dep.userId].balance.toFixed(2)}`);
bot.editMessageText("✅ Approved", { chat_id: id, message_id: q.message.message_id });
}
...
bot.editMessageText("❌ Rejected", { chat_id: id, message_id: q.message.message_id });
}
...
svc = SERVICES[plat][cat].find(s => s.id === sid);
if (svc) break;
}
if (svc) break;
}
...
return bot.sendMessage(id, `🛒 ${svc.name}\nMin: ${svc.min}\nEnter Quantity:`);
}
